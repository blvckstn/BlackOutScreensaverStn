...
## Версионирование
- Текущая: **2.2** (тест: чекбокс «Записывать лог» → `%LocalAppData%\Blackout
  ScreenSaver\test.log` с таймингами; быстрый разбуд `PowerOn` вместо `Wake(mode)` +
  таймауты на каждую DDC‑операцию, чтобы вопросы всегда появлялись и тест не зависал)
- 2.1: выбор метода в окне теста + отсчёт «Пробуждение через N с» крупным числом
